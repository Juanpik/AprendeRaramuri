...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function resetQuizView() {
        ...
        if(quizSetup) quizSetup.style.display = 'block'; // Changed to block for column layout
        if(quizQuestionArea) quizQuestionArea.style.display = 'none';
        if(quizResultsEl) quizResultsEl.style.display = 'none';
        if(retryMissedQuizBtn) retryMissedQuizBtn.style.display = 'none';
    ...
    function initializeApplication() {
        ...
        if(lexiconData.length > 0) {
            ...
            if(memoramaCategorySelect) memoramaCategorySelect.disabled = false; // Habilitar selector Memorama
             ...
             if (flashcardsDataErrorEl && flashcardsSetupControls) { flashcardsSetupControls.style.display = 'flex'; flashcardsDataErrorEl.textContent = noDataMsg; flashcardsDataErrorEl.style.display = 'block'; }
             if (quizDataErrorEl && quizSetup) { quizSetup.style.display = 'block'; quizDataErrorEl.textContent = noDataMsg; quizDataErrorEl.style.display = 'block'; }
             if (memoramaDataErrorEl && memoramaSetup) { memoramaSetup.style.display = 'flex'; memoramaDataErrorEl.textContent = noDataMsg; memoramaDataErrorEl.style.display = 'block';}
        }
