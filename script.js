...
document.addEventListener('DOMContentLoaded', () => {
   ...
   // =============================================
    ...
    function createMemoramaFaceContent(cardInfo, faceElement) { // Reutilizada para Flashcards
        if (!cardInfo || !faceElement) {
            console.error("[Memorama/Flashcard Critico] Faltan parámetros en createMemoramaFaceContent"); return;
        }
        faceElement.innerHTML = ''; // Limpiar
        // console.log(`[Memorama/Flashcard DEBUG] Creando contenido para Cara (ID: ${cardInfo.id}, Tipo: ${cardInfo.type})`);
        try {
            if (cardInfo.type === 'image' && cardInfo.value) {
                const img = document.createElement('img');
                img.src = cardInfo.value;
                img.alt = cardInfo.altText || "Imagen"; // Alt text más genérico
                img.loading = 'lazy';
                img.onerror = function() {
                    console.error(`[Memorama/Flashcard Critico] Falló carga IMG: ${this.src} (ID: ${cardInfo.id})`);
                ...
                console.warn(`[Memorama/Flashcard Warn] Contenido inválido (ID: ${cardInfo.id}):`, cardInfo);
            ...
            console.error("[Memorama/Flashcard Critico] Excepción en createMemoramaFaceContent:", e, cardInfo);
            try { faceElement.innerHTML = '<p style="color:red; font-size:10px;">Error JS!</p>'; } catch (fe) {}
        }
    }

    function prepareCardData(requestedPairs) { // Específico Memorama
    ...
    function buildMemoramaGrid() { // Específico Memorama
        ...
        mCards.forEach((cardData, index) => {
            ...
            createMemoramaFaceContent(cardData, frontFace); // Rellenar cara frontal

            const backFace = document.createElement('div');
            backFace.classList.add('card-face', 'card-back');
            // El ::before CSS pone el '?'
    ...
    function startMemorama(numPairs) { // Específico Memorama
        ...
        itemsForGame.forEach(item => {
            // Cara frontal es el contenido (imagen o texto raramuri)
            // Memorama específico: Una tarjeta es la imagen, la otra el texto rarámuri
    ...
    function handleMemoramaCardClick(event) { // Específico Memorama
        ...
        if (clickedCardElement.classList.contains('flipped') || clickedCardElement.classList.contains('matched')) {
            console.log("[Memorama] Click ignorado."); return;
        }

        clickedCardElement.classList.add('flipped');
        mFlippedElements.push(clickedCardElement);
        console.log(`[Memorama] Cartas volteadas: ${mFlippedElements.length}`);
    ...
    function checkMemoramaMatch() { // Específico Memorama
    ...
    function setupMemoramaControls() { // Específico Memorama
        if (!memoramaSetup || !resetMemoramaBtn || difficultyButtons.length === 0) {
            console.error("[Memorama Critico] Faltan elementos HTML para controles de Memorama."); return;
        }
        console.log("[Memorama] Configurando controles.");
        difficultyButtons.forEach(button => {
            button.addEventListener('click', () => {
                const pairs = parseInt(button.getAttribute('data-pairs'));
                if (isNaN(pairs) || pairs <= 0) { console.error("[Memorama Error] Atributo data-pairs inválido:", button); return; }
                difficultyButtons.forEach(btn => btn.classList.remove('selected'));
    ...
    // ========= SECCIÓN QUIZ ======================
    // =============================================
    // Funciones getWrongOptions, generateQuizQuestions, startQuiz, displayQuestion,
    // handleMCAnswer, handleTextAnswer, goToNextQuestion, showResults, resetQuizView,
    // setupQuizControls - SIN CAMBIOS respecto a la versión anterior completa.
    // (Se mantienen igual que en la respuesta que incluía Flashcards por primera vez)
    ...
    function startQuiz(isRetry = false) {
         ...
         if (!isRetry) {
             ...
             missedQuestions = [];
             if (currentQuizSet.length === 0) { alert("¡Felicidades! No hubo preguntas falladas en la ronda anterior."); resetQuizView(); return; }
         ...
         if (!currentQuizSet || currentQuizSet.length === 0) {
             ...
             if(quizDataErrorEl) {
                 ...
                 if (!quizDataErrorEl.textContent) { quizDataErrorEl.textContent = "No se pudieron generar preguntas."; quizDataErrorEl.style.display = 'block'; }
             }
    ...
    function displayQuestion() {
        ...
        if (!q || !q.type || !q.question || typeof q.answer === 'undefined') { console.error("[Quiz Error] Pregunta inválida encontrada:", q); goToNextQuestion(); return; }
        ...
        if(quizFeedbackEl) { quizFeedbackEl.textContent = ''; quizFeedbackEl.className = ''; } // Resetear feedback
        if(nextQuestionBtn) nextQuestionBtn.style.display = 'none';
        if (q.image && quizImageContainer) {
            const img = document.createElement('img'); img.src = q.image; img.alt = `Imagen para la pregunta`;
            img.loading = 'lazy'; img.onerror = function() { console.error(`[Quiz Error] Imagen no cargada: ${this.src}`); this.alt = 'Error al cargar imagen'; this.src='images/placeholder.png'; };
            quizImageContainer.appendChild(img);
        }
        if (q.type.startsWith('MC_') && quizOptionsEl) {
            quizOptionsEl.style.display = 'block';
            if (!q.options || q.options.length < 2) { console.error("[Quiz Error] Pregunta MC sin opciones:", q); quizOptionsEl.innerHTML = '<p style="color:var(--error-red);">Error: Opciones no disponibles.</p>'; quizActive = false; if(nextQuestionBtn) nextQuestionBtn.style.display = 'inline-block'; }
    ...
    function handleMCAnswer(event) {
        ...
        quizActive = false; const selectedButton = event.target; const selectedAnswer = selectedButton.textContent;
        ...
        if (selectedAnswer === correctAnswer) {
            ...
            quizFeedbackEl.innerHTML = `Incorrecto. La respuesta correcta es: <strong>${correctAnswer}</strong>`; quizFeedbackEl.className = 'incorrect';
    ...
    function handleTextAnswer() {
         ...
         if (userAnswer === correctAnswer && userAnswer !== '') {
             score++; quizTextAnswerInput.classList.add('correct');
             quizFeedbackEl.textContent = '¡Correcto!'; quizFeedbackEl.className = 'correct';
         } else {
             quizTextAnswerInput.classList.add('incorrect');
             quizFeedbackEl.innerHTML = `Incorrecto. La respuesta correcta es: <strong>${originalCorrectAnswer}</strong>`; quizFeedbackEl.className = 'incorrect';
             if (currentQuestion.item && !missedQuestions.some(mq => mq.item.id === currentQuestion.item.id)) { missedQuestions.push(JSON.parse(JSON.stringify(currentQuestion))); }
         }
         nextQuestionBtn.style.display = 'inline-block';
     }
    ...
    function resetQuizView() {
        ...
        if(quizSetup) quizSetup.style.display = 'block'; if(quizQuestionArea) quizQuestionArea.style.display = 'none';
        if(quizResultsEl) quizResultsEl.style.display = 'none'; if(retryMissedQuizBtn) retryMissedQuizBtn.style.display = 'none';
        if(quizDataErrorEl) quizDataErrorEl.style.display = 'none'; if(quizImageContainer) quizImageContainer.innerHTML = '';
        if(quizFeedbackEl) { quizFeedbackEl.textContent = ''; quizFeedbackEl.className = ''; } if(quizOptionsEl) quizOptionsEl.innerHTML = '';
        if(quizTextAnswerInput) { quizTextAnswerInput.value = ''; quizTextAnswerInput.className = ''; } if(quizQuestionEl) quizQuestionEl.textContent = '';
    ...
    function setupQuizControls() {
        if (!startQuizBtn || !nextQuestionBtn || !restartQuizBtn || !retryMissedQuizBtn || !submitTextAnswerBtn || !quizTextAnswerInput || !quizLengthSelect) {
            console.error("[Quiz Error] Faltan elementos de control del Quiz en el DOM."); return;
        }
    ...
    function prepareFlashcardData() {
        ...
        flashcardData = shuffleArray([...validLexicon]);
    ...
    function displayCurrentFlashcard() {
        if (!flashcardData || flashcardData.length === 0 || !flashcardAreaEl || flashcardAreaEl.style.display === 'none') {
            console.log("[Flashcards] No hay datos o área no visible."); return;
        }
        if (currentFlashcardIndex < 0 || currentFlashcardIndex >= flashcardData.length) {
            console.error(`[Flashcards] Índice inválido: ${currentFlashcardIndex}. Reseteando a 0.`); currentFlashcardIndex = 0;
        }
        ...
        if (!cardData) {
            ...
            if (flashcardsErrorEl) { flashcardsErrorEl.textContent = 'Error al cargar datos de la tarjeta.'; flashcardsErrorEl.style.display = 'block'; flashcardAreaEl.style.display = 'none'; }
            return;
        }
        ...
        if (flashcardFrontEl) {
            ...
            if (cardData.image) {
                const img = document.createElement('img'); img.src = cardData.image; img.alt = cardData.spanish || 'Flashcard Image';
                img.loading = 'lazy'; img.onerror = function() { this.alt='Error img'; this.src='images/placeholder.png'; };
                flashcardFrontEl.appendChild(img);
                if (cardData.spanish) {
                    const p = document.createElement('p'); p.textContent = cardData.spanish;
                    p.style.fontSize = '0.85em'; // Aplicar estilo desde JS
                    // QUITAR: p.style.marginTop = 'var(--space-sm)'; <= Asegurar que no esté
    ...
    function setupFlashcardsControls() {
        if (!flashcardEl || !prevFlashcardBtn || !flipFlashcardBtn || !nextFlashcardBtn || !shuffleFlashcardsBtn) { console.error("Faltan elementos de control de Flashcards en el DOM."); return; }
